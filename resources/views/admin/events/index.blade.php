...
<div class="mb-6 flex flex-col gap-4">
    ...
    <form method="GET" action="{{ route('admin.events.index') }}" class="flex items-center gap-2 w-full">
        <div class="relative flex-1">
            <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Cari event..." class="w-full px-4 py-3 rounded-2xl border border-slate-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 outline-none transition text-sm">
        </div>
        ...
        @if($search)
        <a href="{{ route('admin.events.index') }}" class="px-4 py-3 border border-slate-200 text-slate-600 rounded-2xl hover:bg-slate-50 transition text-sm font-medium whitespace-nowrap">
            Reset
        </a>
        @endif
    </form>
